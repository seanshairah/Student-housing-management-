...
A complete, production-minded MVP for managing two student houses — **Mufudzi House** and **Siphiwe House**. It combines a beautiful animated public marketing site, an online booking/application workflow, and role-based dashboards for owners, students, and caretakers — with payments, email, SMS, invoices, receipts, statements, reporting, and a communication center.
...
- **Public marketing website** — animated hero, house showcase, interactive comparison, "How it works" timeline, amenities, FAQs, contact, and a multi-step booking form.
...
- **Owner dashboard** — overview KPIs & charts, houses, rooms, applications (approve/reject), students, payments, invoices, reports, services & caretakers, communication center, settings.
...
| SMS | Provider-adapter pattern (BulkSMS-ready) |
...
│   └── seed.ts                # Seed data (houses, rooms, users, applications, payments…)
...
```
All third-party providers (Resend, SMTP, BulkSMS, Paynow) are **optional** — the app runs fully in development/mock mode without them.
...
| Role | Email | Password |
| --- | --- | --- |
| Owner / Admin | `owner@studenthousing.local` | `owner123` |
| Student | `student@studenthousing.local` | `student123` |
| Caretaker | `caretaker@studenthousing.local` | `caretaker123` |
...
1. **Browse & apply** — open `/`, explore both houses, click **Book a Room**, pick an available room, and submit. You'll get a confirmation with a reference. The room status flips to *Pending Application* (no longer freely bookable).
...
Double-booking is prevented with **server-side transactions**: the chosen room is re-checked inside the transaction and only reserved if still available.
...
Marketing animations are driven by a central, accessible config: `src/lib/animation-config.ts` (with a JSON spec at `src/data/animation-config.json`). It defines reusable Framer Motion variants and per-section motion rules, and everything respects `prefers-reduced-motion`.
...
- A single owner-managed settings singleton (multi-tenant is out of scope for the MVP).
...
Built with care as a real, modern student-housing platform — not a school project.
