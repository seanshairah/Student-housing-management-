...
async function main() {
  ...
  await prisma.settings.create({
    data: {
      id: "singleton",
      businessName: "Student Housing Co.",
      ownerName: "Tatenda Moyo",
      ownerEmail: "owner@studenthousing.local",
      ...
      paymentTermsDays: 7,
      defaultMessage: "Thank you for being part of our student housing community.",
    },
  });
  ...
  const owner = await prisma.user.create({
    data: {
      email: "owner@studenthousing.local",
      name: "Tatenda Moyo",
      phone: "+263770000001",
      role: "OWNER",
  ...
  const mufudzi = houseRecords["mufudzi"];
  const siphiwe = houseRecords["siphiwe"];

  // ── Caretakers ──
  const caretakerUser = await prisma.user.create({
    data: {
      email: "caretaker@studenthousing.local",
      name: "Joseph Banda",
      phone: "+263770000010",
      role: "CARETAKER",
  ...
  const caretaker1 = await prisma.caretaker.create({
    data: {
      ...
      role: "Head Caretaker",
      houseId: mufudzi.id,
      notes: "Handles Mufudzi House day-to-day.",
    },
  });
  await prisma.caretaker.create({
    data: {
      name: "Grace Ncube",
      phone: "+263770000011",
      email: "grace.caretaker@studenthousing.local",
      role: "Caretaker",
      houseId: siphiwe.id,
      notes: "Handles Siphiwe House day-to-day.",
    },
  });
  ...
  const demoStudent = await prisma.user.create({
    data: {
      email: "student@studenthousing.local",
  ...
  const demoRoom = mufudzi.rooms[0];
  ...
  for (const s of extraStudents) {
    ...
    const room = s.house.rooms[s.roomIdx];
  ...
  for (const a of apps) {
    appCounter++;
    const room = a.house.rooms[a.roomIdx];
  ...
  await prisma.serviceRequest.create({
    data: {
      ...
      description: "The cold water tap in the Mufudzi communal kitchen drips constantly.",
  ...
  console.log("\nLogin accounts:");
  console.log("  Owner:     owner@studenthousing.local / owner123");
  console.log("  Student:   student@studenthousing.local / student123");
  console.log("  Caretaker: caretaker@studenthousing.local / caretaker123");
}
